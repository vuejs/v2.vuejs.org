...
When the page loads, that element gains focus (note: autofocus not works on mobile safari). In fact, if you haven't clicked on anything else since visiting this page, the input above should be focused now. Now let's build the directive that accomplishes this:
