...
Note that in Vue 2.x, the primary form of code reuse and abstraction is **components**. However, there may be cases where you need some low-level DOM access on plain elements, and this is where custom directives really become useful.
...
When the page loads, that element gains focus. (**Note:** `autofocus` doesn't work on mobile Safari.) In fact, if you haven't clicked on anything else since visiting this page, the `<input>` above should be focused now.
...
We'll explore the arguments passed into these hooks (that is, `el`, `binding`, `vnode`, and `oldVnode`) in the next section.
...
This would pin the element `200px` from the top of the page. But what happens if you run into a scenario when you need to pin the element from the left, instead of the top?

Here's where a dynamic argument that can be updated per-component instance comes in very handy:
