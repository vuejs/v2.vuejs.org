...
When used together with `v-if`, `v-for` has a higher priority than `v-if`. Its recommended to use computed properties and `v-for`. See the [list rendering guide](/v2/guide/list.html#v-for-with-v-if) for details.
