...
In that case, you can use `v-if` on a `<template>` element, which serves as an invisible wrapper. The final rendered result will not include the `<template>` element.
...
This isn't always desirable, though. That's why Vue offers a way for you to say, "These two elements are completely separate&mdash;don't re-use them."
