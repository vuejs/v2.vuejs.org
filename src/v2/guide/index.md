...
Note that we no longer have to interact with the HTML directly. A Vue app attaches itself to a single DOM element (`#app` in our case) then fully controls it. That single DOM element is our entry point to the page. Everything in the element is now handled by Vue. 
