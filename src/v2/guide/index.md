...
Note that we no longer have to directly interact with the HTML. A Vue app attaches itself to a single, global DOM element (`#app` in our case) then fully controls it. The HTML is our entry point, but everything else happens within the newly created Vue Instance.
