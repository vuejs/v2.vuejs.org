...
3. `v-enter-to`: **Only available in versions 2.1.8+.** Ending state for enter. Added one frame at enter transitions finished's last frame, removed when transition/animation finishes.
...
6. `v-leave-to`: **Only available in versions 2.1.8+.** Ending state for leave. Added one frame at leave transitions finished's last frame, removed when the transition/animation finishes.
